...
- **API version:** Cartesia's API is dated (`Cartesia-Version: YYYY-MM-DD`). Pin **one** version across your services and treat bumping it like a major dependency upgrade — a new version can carry breaking changes — so it's a fixed choice, not a runtime/per-call variable.
  ...
  - **Calling the API directly (no SDK):** you must send `Cartesia-Version` yourself. Pick one date (see [API conventions](https://docs.cartesia.ai/use-the-api/api-conventions.md) for valid dates and what changed between them) and use it everywhere. Browser WebSockets can't set handshake headers, so pass it as **`?cartesia_version=...`** (query wins if both are present).
